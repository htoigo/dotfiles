...
The =jump-char= package gives us an iy-go-to-char enhancement, and acts like
the `f' key command in Vim. Rather than =M-m=, =M-M=, and =s-m= as Magnar Sveen
uses for this, let's try binding the key-chords fg and df.

#+BEGIN_SRC emacs-lisp
  (use-package jump-char
    :chords (("fg" . jump-char-forward)
             ("df" . jump-char-backward)))
#+END_SRC
