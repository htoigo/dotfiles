...
** Use =which-key=
 ...
 Fix paging through completion keys for <F1>.

 Since <F1> C-h is bound by default to =help-for-help=, in order to allow C-h
 to be used for paging through the completions, we must explicitly override the
 binding with:
